...
var WhoItem = React.createClass({
  ...
  render: function() {
    const { groups, clusters, setValue, id, value, intl, noAll } = this.props;
    //const who = getValue();
...
var WhereItem = React.createClass({
  ...
  render: function() {
    const { setValue, clusters, value, noAll, all, id, intl, geojson } = this.props;
...
var WhenItem = React.createClass({
  ...
  render: function() {
    const { setValue, id, value } = this.props;
    //const when = getValue();
      ...
      <div>
        <bs.Col md={4}>
          <bs.ButtonGroup vertical block>
            ...
            <bs.Button bsStyle={value.value === 'custom' ? 'primary' : 'default'} style={{marginBottom: 10}} onClick={() => this.setState({showModal: true})}>Custom</bs.Button>
          </bs.ButtonGroup>
        </bs.Col>
