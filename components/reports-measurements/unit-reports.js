...
class _Report extends React.Component {
  ...
  static _propsToState(props) {
    ...
    var { unit } = cls;
    var { now, report: { level, startsAt, duration: [, unit1] }, series } = props;
    ...
    keys.map(pairWithNext).forEach(([k1, k2]) => {
      var [t1e] = _.last(data.get(k1).points);
  ...
  _renderForecastingChart(viewProps) {
    ...
    var { report: { level }, field, uom } = this.props;
  ...
  _renderSimpleChart(viewProps) {
    ...
    const { defaults, momentToKey } = this.constructor;
    var { formatDate, field, uom } = this.props;
    var { moment0, data } = this.state;

    var k0 = momentToKey(moment0), data0, data0p1;
    if (data) {
      data0 = data.get(k0);
  ...
  _renderComparisonChart(viewProps) {
    ...
    const { defaults, momentToKey } = this.constructor;
    var { report: { duration: [K] }, formatDate, field, uom } = this.props;
